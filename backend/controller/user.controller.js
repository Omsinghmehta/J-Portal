...
export const updateProfile = async (req, res) => {
  try {
    ...
    let resumeUri, photoUri;

    if (resumeFile) resumeUri = getDataUri(resumeFile);
    if (photoFile) photoUri = getDataUri(photoFile);

    let cloudResume, cloudPhoto;

    // Upload resume (PDF)
    if (resumeUri) {
      cloudResume = await cloudinary.uploader.upload(resumeUri.content, {
        resource_type: "raw",
        format: "pdf",
      });
    }
    ...
    if (photoUri) {
      cloudPhoto = await cloudinary.uploader.upload(photoUri.content, {
    ...
    if (cloudPhoto) {
      user.profile.photo = cloudPhoto.secure_url;
    }
